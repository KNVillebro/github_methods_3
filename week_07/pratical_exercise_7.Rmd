...
np.random.seed(7)
#create y_hat values
y_noise = x+ np.random.normal(0, 5, len(x))

#plotting the stuff 
plt.scatter(x, y_noise, label ="points")
plt.plot(x, square(x), label ="True function")
plt.show()
plt.clf()
```
...
```{python}
#Doing some linear regerssion 
regressor = LinearRegression()
fit = regressor.fit(x.reshape(-1,1), y_noise)
y_hat = fit.intercept_ + fit.coef_ * x 

#Plotting the fitting line 
plt.scatter(x, y_noise, label ="points")
plt.plot(x, square(x), label ="True function")
plt.plot(x, y_hat, label = "linear reression")
plt.show()
plt.clf()
